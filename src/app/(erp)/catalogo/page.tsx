...
export default async function CatalogoPage() {
  ...
  interface LinhaRaw { id: string; nome: string; ordem: number; grupo: string | null; grupo_ordem: number | null }

  const linhas = (linhasRaw as LinhaRaw[] ?? [])
    .map((l) => ({ ...l, count: contagem[l.id] ?? 0 }))
    .filter((l) => l.count > 0);

  return <CatalogoIndex linhas={linhas} />;
}
